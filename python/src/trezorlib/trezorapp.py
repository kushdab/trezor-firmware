...
class AppImage:
    ...
    header_bytes: bytes
    # Image payload (elf file, or other binary format)
    payload: bytes
    ...
    def calc_hash(self) -> bytes:
        """Calculate the SHA256 hash of the app payload."""
        return sha256(self.header_bytes).digest()

    def get_chunks(self) -> list[tuple[bytes, bytes]]:
        ...
        assert (
            hash == self.header.payload_hash
        ), "Calculated payload hash does not match header"
...
def load(
    ...
    if min_version is not None:
        ...
        min_version_info = ""
        min_version = image.header.version[:2]

    print(f"Requesting {image.header.id} {min_version_info}")

    hash = image.calc_hash() if force_reload else b""

    # Send a request to the device to load the app, providing the hash, app ID, and minimum version.
    resp = session.call(
        messages.TrezorAppLoad(hash=hash, id=image.header.id, version=min_version)
    )
