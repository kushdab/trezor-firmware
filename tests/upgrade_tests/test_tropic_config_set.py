...
def _set_slot(config: dict, slot: int, value: int | None) -> None:
    ...
    r_user_data[slot] = {"value": value.to_bytes(4, "little")}
...
def _check_tropic_model_output(
    ...
    ) == expected_distribution_version.to_bytes(4, "little")
...
@pytest.mark.parametrize(
    "scenario",
    [pytest.param(scenario, id=scenario.id) for scenario in TROPIC_BOOT_SCENARIOS],
)
